...
const clean = s => sanitizeHtml(String(s||''), { allowedTags: [], allowedAttributes: {} }).trim();
const requireAuth = (req,res,next)=> req.session?.uid ? next() : res.status(401).json({error:'auth_required'});
async function userRow(uid){ const {rows}=await q('select id,email,name,handle,avatar,bio,created_at from users where id=$1',[uid]); return rows[0]||null; }
...
function pickOrigins(listOrSingle){
  ...
  return (origin,cb)=>{ if(!origin) return cb(null,true); if(list.includes(origin)) return cb(null,true); cb(new Error('Not allowed by CORS: '+origin)); };
}
...
app.post('/api/signup', async (req,res)=>{
  try{
    ...
    if(!email || !pass || !/^[a-z][a-z0-9._-]{2,19}$/.test(handle)) return res.status(400).json({error:'bad_input'});
    const id = uuid();
    const hash = await bcrypt.hash(pass,10);
    await q('insert into users(id,email,pass_hash,name,handle) values($1,$2,$3,$4,$5)', [id,email,hash,name,handle]);
    req.session.uid=id;
    res.json({ ok:true, me: await userRow(id) });
  }catch(e){
    const m=e.message||'';
    if(m.includes('users_email_key')) return res.status(409).json({error:'email_taken'});
    if(m.includes('users_handle_key'))return res.status(409).json({error:'handle_taken'});
...
app.post('/api/login', async (req,res)=>{
  ...
  const ok=await bcrypt.compare(pass,u.pass_hash); if(!ok) return res.status(401).json({error:'invalid'});
  req.session.uid=u.id;
  res.json({ ok:true, me: normalizeUser(u) });
});
app.post('/api/logout', (req,res)=> req.session.destroy(()=>res.json({ok:true})));
app.get('/api/me', async (req,res)=> res.json({ me: req.session?.uid ? await userRow(req.session.uid) : null }));
...
app.post('/api/handle/update', requireAuth, async (req,res)=>{
  ...
  try{ await q('update users set handle=$1 where id=$2',[h,req.session.uid]); res.json({ok:true,handle:h}); }
  catch{ res.status(409).json({error:'handle_taken'}); }
});
...
app.post('/api/posts/:id/comment', requireAuth, async (req,res)=>{
  const id=uuid();
  const body=clean(req.body.body||''); if(!body) return res.status(400).json({error:'empty'});
...
const run = async ()=>{ await ensureTables(); app.listen(PORT, ()=>console.log('Chatternet backend on :'+PORT)); };
if(process.argv[2]==='initdb'){ ensureTables().then(()=>{ console.log('DB ready'); process.exit(0); }); } else { run(); }
